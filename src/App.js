    ...
    jobs: [
      {
        ...
        description: "Node.js in-house & customized training for particulars and teams.",
      }
    ]
  }, {
    image: logoBeeva,
...
function App() {
  return (
    <div class="mx-auto bg-cyan-900 w-screen">
      <div class="absolute z-20 top-0 inset-x-0 flex justify-center overflow-hidden pointer-events-none">
        <div class="w-[108rem] flex-none flex justify-end">
          <picture>
            <img src={light1} alt="" class="w-[71.75rem] flex-none max-w-none dark:hidden" decoding="async" />
          </picture>
          <picture>
            <img src={light2} alt="" class="w-[90rem] flex-none max-w-none hidden dark:block" decoding="async" />
          </picture>
        </div>
      </div>
...
Sidebar
      ...
      */}
        <main role="main" class="w-full h-full flex-grow p-10 overflow-auto">
          <ProfileCard
            avatar={avatar}
          ...
          <TimelineContainer>
            {experience.map(({ image, date, company, title, jobs, description }, i) => (
              <TimelineCard
                key={title}
                image={image}
                date={date}
